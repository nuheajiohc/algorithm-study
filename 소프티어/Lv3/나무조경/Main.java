...
public class Main {
    ...
    public static int DFS(int pairCount, int start, int currentBeauty){
        if(pairCount == 4){
            return currentBeauty;
        }

        int maxBeauty = currentBeauty;
        for(int i=start; i<n*n; i++){
            int x = i/n;
            int y = i%n;
            if(vis[x][y]) continue;
            ...
            for(int dir=0; dir<2; dir++){
                ...
                if(vis[nx][ny]) continue;
                vis[nx][ny]=true;
                maxBeauty = Math.max(maxBeauty, DFS(pairCount+1, i+1, currentBeauty+board[x][y]+board[nx][ny]));
                vis[nx][ny]=false;
            }
            vis[x][y]=false;
        }
